...
#define STR(x) _STR(x)
                   ...
                   const std::function<void()> &audioCallback)
  {
    ...
    std::vector<std::string> fillerWords = {"ah", "uhm", "um", "er", "uh"};

    for (const auto &filler : fillerWords)
    {
      size_t pos = std::string::npos;
      while ((pos = text.find(filler)) != std::string::npos)
      {
        text.erase(pos, filler.length());
      }
    }
    ...
    std::map<Phoneme, float> punctuationSilenceSeconds = {
      ...
      if (voice.synthesisConfig.phonemeSilenceSeconds)
      {
             ...
             sentencePhonemesIter++)
        {
          ...
          if (phonemeSilenceSeconds.count(currentPhoneme) > 0 ||
              punctuationSilenceSeconds.count(currentPhoneme) > 0)
          {
            float silenceDuration = phonemeSilenceSeconds.count(currentPhoneme)
                                        ? phonemeSilenceSeconds[currentPhoneme]
                                        : punctuationSilenceSeconds[currentPhoneme];

            phraseSilenceSamples.push_back(
                (std::size_t)(silenceDuration *
